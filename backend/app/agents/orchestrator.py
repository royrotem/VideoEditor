...
class Orchestrator:
    ...
    async def analyze_asset(self, asset_input: object) -> AssetFacts:
        """Run the Vision Analyzer over a single asset."""
        return await self._run_agent(
            self.AGENT_VISION_ANALYZER, asset_input, AssetFacts
        )
    ...
    async def plan_edit(
        ...
        *,
        ...
        return await self._run_agent(
            self.AGENT_EDITING_PLANNER, payload, EditDecisionList
        )

    async def review_edl(
        self, *, brief: BriefPlan, edl: EditDecisionList
    ) -> QAReport:
    ...
    async def _run_agent(
        ...
        if not isinstance(agent, _Agent):
            raise AppError(
                f"agent {agent_name!r} is not a single-shot Agent"
            )
